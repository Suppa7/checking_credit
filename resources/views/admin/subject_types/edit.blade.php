...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active" aria-current="page">แก้ไขข้อมูลหมวดวิชา (Subject Type)</li>
@endsection
        ...
        <div class="col-md-8">
            <div class="card shadow-sm border-0 rounded-4">
                <div class="card-header bg-white border-bottom-0 pt-4 pb-0 px-4">
                    <h3 class="mb-0">แก้ไขข้อมูลหมวดวิชา (Subject Type)</h3>
                </div>
                <div class="card-body p-4">
                    <form action="{{ route('admin.subject_types.update', $subject_type) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('subject_category_id') is-invalid @enderror" id="subject_category_id" name="subject_category_id" required>
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('subject_category_id', $subject_type->subject_category_id) == $category->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('type_name') is-invalid @enderror" id="type_name" name="type_name" value="{{ old('type_name', $subject_type->type_name) }}" required>
                            @error('type_name')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="mb-4">
                            ...
                            <input type="number" min="0" class="form-control @error('credit_needed') is-invalid @enderror" id="credit_needed" name="credit_needed" value="{{ old('credit_needed', $subject_type->credit_needed) }}" required>
                            @error('credit_needed')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
