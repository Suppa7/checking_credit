                        ...
                        <tbody>
                            @forelse($users as $user)
                                <tr>
                                    ...
                                    <td class="px-4 py-3 align-middle text-center">
                                        <div class="fw-bold small">{{ $user->student->curriculum->major->major_name_thai }}
                                            {{ $user->student->curriculum->curriculum_year }}</div>
                                    </td>
                                    <td class="px-4 py-3 align-middle text-center small text-muted">
                                        {{ $user->created_at->format('d/m/Y H:i') }}</td>

                                    <td class="px-4 py-3 text-end">
                                        <a href="{{ route('admin.user_managements.edit', $user) }}"
                                            class="btn btn-sm btn-outline-warning">แก้ไข</a>
                                        <form action="{{ route('admin.user_managements.destroy', $user) }}" method="POST"
                                            class="d-inline" onsubmit="return confirm('ยืนยันการลบผู้ใช้?');">
                                            @csrf
                                            @method('DELETE')
