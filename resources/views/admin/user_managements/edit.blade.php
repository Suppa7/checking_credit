        ...
        <div class="col-md-6">
            <div class="card shadow-sm border-0 rounded-4">
                <div class="card-header bg-white py-3 border-0">
                    <h5 class="mb-0 fw-bold">แก้ไขผู้ใช้งาน: {{ $user->student_id }}</h5>
                </div>
                <div class="card-body p-4">
                    <form action="{{ route('admin.user_managements.update', $user) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            <label for="student_id" class="form-label">Student ID / Username</label>
                            <input type="text" name="student_id" id="student_id" class="form-control @error('student_id') is-invalid @enderror" value="{{ old('student_id', $user->student_id) }}" required>
                            @error('student_id')
                        ...
                        <div class="mb-3">
                            <label for="password" class="form-label">Password (เว้นว่างไว้หากไม่ต้องการเปลี่ยน)</label>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="role" id="role" class="form-select @error('role') is-invalid @enderror" onchange="toggleStudentFields()" required>
                                <option value="user" {{ old('role', $user->role) == 'user' ? 'selected' : '' }}>User (นักศึกษา)</option>
                        ...
                        <div id="student_fields" style="display: {{ old('role', $user->role) == 'user' ? 'block' : 'none' }};">
                            <hr class="my-4">
                            <h6 class="fw-bold mb-3 text-primary">ข้อมูลนักศึกษา</h6>
                            
                            <div class="mb-3">
                                <label for="student_name" class="form-label">ชื่อ-นามสกุล</label>
                                <input type="text" name="student_name" id="student_name" class="form-control @error('student_name') is-invalid @enderror" value="{{ old('student_name', $user->student ? $user->student->student_name : '') }}">
                                @error('student_name')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="major_id" id="major_id" class="form-select @error('major_id') is-invalid @enderror">
                                        ...
                                        @foreach($majors as $major)
                                            <option value="{{ $major->id }}" {{ old('major_id', $user->student ? $user->student->major_id : '') == $major->id ? 'selected' : '' }}>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="submajor_id" id="submajor_id" class="form-select @error('submajor_id') is-invalid @enderror">
                                        ...
                                        @foreach($submajors as $submajor)
                                            <option value="{{ $submajor->id }}" {{ old('submajor_id', $user->student ? $user->student->submajor_id : '') == $submajor->id ? 'selected' : '' }}>
                        ...
                        <div class="d-flex justify-content-between mt-4 pb-2">
                            ...
                            <button type="submit" class="btn btn-primary rounded-pill px-4 fw-bold shadow-sm">อัปเดตข้อมูล</button>
                        </div>
                    </form>
                </div>
            </div>
