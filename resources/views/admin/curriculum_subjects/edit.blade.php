                    ...
                    <form action="{{ route('admin.curriculum_subjects.update', $curriculum_subject) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('curriculum_id') is-invalid @enderror" id="curriculum_id" name="curriculum_id" required>
                                <option value="" disabled>-- เลือกเล่มหลักสูตร --</option>
                                @foreach($curriculums as $curriculum)
                                    <option value="{{ $curriculum->id }}" {{ old('curriculum_id', $curriculum_subject->curriculum_id) == $curriculum->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <select class="form-select @error('subject_category_id') is-invalid @enderror" id="subject_category_id" name="subject_category_id" required>
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('subject_category_id', $curriculum_subject->subject_category_id) == $category->id ? 'selected' : '' }}>
